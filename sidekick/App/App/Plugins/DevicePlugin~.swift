...
@objc func stopDiscoverDevices(_ call: CAPPluginCall) {
    ...
    @objc func connectToDeviceAP(_ call: CAPPluginCall) {
        ...
        checkCurrentConnection { isConnected in
            ...
            self.attemptConnection(call: call, bridge: bridge)
        }
    }

    private func checkCurrentConnection(completion: @escaping (Bool) -> Void) {
    ...
    private func attemptConnection(call: CAPPluginCall, bridge: CAPBridgeProtocol) {
        ...
        NEHotspotConfigurationManager.shared.apply(configuration) { error in
            ...
            if #available(iOS 14.0, *) {
                ...
                NEHotspotHelper.register(options: [:], queue: queue) { (command) in
